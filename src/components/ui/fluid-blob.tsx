...
void main() {
    ...
    // Orange to cream gradient colors
    vec3 orangeCore = vec3(1.0, 0.627, 0.0);     // #FFA000
    vec3 orangeMid = vec3(1.0, 0.447, 0.0);      // #FF7200
    vec3 creamGlow = vec3(1.0, 0.941, 0.878);    // #FFF1E0
    vec3 backgroundColor = vec3(0.969, 0.969, 0.969); // Light background
    ...
    if (t > 0.0) {
        ...
        // Create gradient based on position and fresnel
        float distanceFromCenter = length(p.xy) * 0.5;
        vec3 color = mix(orangeCore, orangeMid, distanceFromCenter);
        color = mix(color, creamGlow, fresnel * 0.7);
        
        // Add top-left lighting effect
        vec3 lightDir = normalize(vec3(-1.0, 1.0, 1.0));
        float lightInfluence = dot(normal, lightDir) * 0.5 + 0.5;
        color = mix(color, creamGlow, lightInfluence * 0.3);
